...
printf '%s\n' 'FROM scratch' > "$TEMPORARY/Dockerfile"
printf 'raw scanner output\n' > "$TEMPORARY/evidence/api.json"
git -C "$TEMPORARY" add .quality Dockerfile
...
run_assessment() {
    expect "$1" "$2" --root "$TEMPORARY" --enrollment .quality/container-image-enrollment.json \
        --inventory .quality/images.json --report "${3:-assessment.json}" --exceptions .quality/container-image-exceptions.json \
        --as-of 2026-08-28T12:00:00Z --max-age-hours 2
}

run_assessment 'a fresh immutable assessment succeeds' 0

printf '%s' '{"version":1,"dockerfiles":[{"path":"Dockerfile","classification":"build"}],"image_sources":[{"id":"api","dockerfile":"MissingDockerfile"}]}' > "$TEMPORARY/.quality/container-image-enrollment.json"
git -C "$TEMPORARY" add .quality/container-image-enrollment.json
enrollment_sha256="$(shasum -a 256 "$TEMPORARY/.quality/container-image-enrollment.json" | cut -d ' ' -f 1)"
clean_assessment
run_assessment 'an invalid enrollment graph fails' 1

printf '%s' '{"version":1,"dockerfiles":[{"path":"Dockerfile","classification":"build"}],"image_sources":[{"id":"api","dockerfile":"Dockerfile"}]}' > "$TEMPORARY/.quality/container-image-enrollment.json"
git -C "$TEMPORARY" add .quality/container-image-enrollment.json
enrollment_sha256="$(shasum -a 256 "$TEMPORARY/.quality/container-image-enrollment.json" | cut -d ' ' -f 1)"
clean_assessment
run_assessment 'an absolute report path fails' 1 "$TEMPORARY/assessment.json"
ln -s assessment.json "$TEMPORARY/assessment-link.json"
run_assessment 'a symlinked report fails' 1 assessment-link.json
...
printf '%s' '{"version":2,"exceptions":[]}' > "$TEMPORARY/.quality/container-image-exceptions.json"
git -C "$TEMPORARY" add .quality/container-image-exceptions.json
printf '\n' >> "$TEMPORARY/.quality/container-image-enrollment.json"
working_tree_enrollment_sha256="$(shasum -a 256 "$TEMPORARY/.quality/container-image-enrollment.json" | cut -d ' ' -f 1)"
assessment "{\"version\":2,\"enrollment_sha256\":\"$working_tree_enrollment_sha256\",\"scans\":[{\"image_id\":\"api\",\"artifact_digest\":\"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\",\"scanned_at\":\"2026-08-28T11:00:00Z\",\"raw_evidence\":{\"path\":\"evidence/api.json\",\"sha256\":\"$raw_sha256\"}}],\"findings\":[]}"
run_assessment 'a policy working-tree and index mismatch fails' 1
