...
mkdir "$FAKE_BIN"
printf '#!/bin/sh\nlines=\noutput=\nwhile [ "$#" -gt 0 ]; do\n    if [ "$1" = "--min-lines" ]; then\n        lines="$2"\n    fi\n    if [ "$1" = "--output" ]; then\n        output="$2"\n    fi\n    shift\ndone\nmkdir -p "$output"\nif [ "$lines" = 1 ]; then\n    printf "{\\\"statistics\\\": {\\\"total\\\": {\\\"sources\\\": 18}}, \\\"duplicates\\\": %%s}" "$SCOPE_CLONES" > "$output/jscpd-report.json"\n    exit 1\nfi\nprintf "{\\\"statistics\\\": {\\\"total\\\": {\\\"sources\\\": 18}}, \\\"duplicates\\\": []}" > "$output/jscpd-report.json"\n' > "$FAKE_BIN/jscpd"
...
if ! diff \
    <(grep '^- id:' "$CONSUMER/.pre-commit-hooks.yaml" | cut -d ' ' -f3 | grep -v -e '^check-forbidden-mocks$' -e '^check-pytest-describe$' -e '^check-hook-scope-contract$' -e '^check-manifest-audit-coverage$' -e '^check-dockerfile-enrollment$' | sort) \
...
for hook in check-forbidden-mocks check-pytest-describe check-hook-scope-contract check-manifest-audit-coverage check-dockerfile-enrollment; do
